...
export const commentService = {
    ...
    async update(id: string, commentData: CommentInputModel, userId: string): Promise<ServiceResult<CommentDbModel>>{
        ...
        const updatedComment = await commentRepository.update(id, commentData, userId);
        if (!updatedComment){
            return {data: null, status: false, msg: SERVICE_CUSTOM_MSG.NOT_FOUND}
        }
    ...
    async create(
        ...
        const createdComment = await commentRepository.create(commentInputData, user, postId);
        if (!createdComment){
            return {data: null, status: false, msg: SERVICE_CUSTOM_MSG.NOT_FOUND}
        }
    ...
    async findManyByPostId(
        ...
        const comments = await commentRepository.findManyByPostId(postId, sortBy, sortDir, page, size);
        if (!comments){
            return {data: null, status: false, msg: SERVICE_CUSTOM_MSG.NOT_FOUND}
        }
    ...
    async deleteById(postId: string, userId: string): Promise<ServiceResult<CommentDbModel>>{
        ...
        const deleteResult = await commentRepository.delete(postId, userId);
        if (!deleteResult){
            return {data: null, status: false, msg: SERVICE_CUSTOM_MSG.ACCESS_DENIED}
        }
