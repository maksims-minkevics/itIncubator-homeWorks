...
export class CommentController {
   static async getById (req: Request, resp: Response) {
       try {
           const comment = await commentService.find(req.params.id as string)
           if (!comment.status) {
               return resp
    ...
    static async updateById (req: Request, resp: Response){
        const isUpdated = await commentService.update(
    ...
    static async deleteById (req: Request, resp: Response) {
        try {
            const isDeleted = await commentService.deleteById(
                req.params.id as string,
                req.user.userId);
