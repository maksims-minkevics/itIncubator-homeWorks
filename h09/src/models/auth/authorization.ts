...
export const jwtRefreshTokenAuth= async (req: Request, resp: Response, next: NextFunction) => {
    ...
    const result = await refreshTokenService.verify(inputToken);
    if (!result.status){
        return resp
            .status(HTTP_STATUS.UNAUTHORIZED)
            .end();
...
export const customBasicAuth =
    async (req: Request, resp: Response, next: NextFunction) => {
        ...
        const user = await userService.findByEmailOrLoginDbModel(
        ...
        const isAuthorized =
            ...
            && (await jwtTokenService.compare(user.data.password, authHeader.password)).status
