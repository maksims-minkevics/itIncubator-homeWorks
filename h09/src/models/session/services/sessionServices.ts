...
export const sessionService = {
    ...
    async getActiveSessions(userId: string): Promise<ServiceResult<SessionViewModel[]>> {
        try {
            const activeSessions = await sessionRepository.findActiveSessions(userId);
            if (!activeSessions){
    ...
    async deleteAllExceptCurrent(deviceId: string, userId: string): Promise<ServiceResult<SessionDbModel>> {
        try {
            const isDeleted = await sessionRepository.deleteAllExceptCurrent(deviceId, userId);
    ...
    async deleteById(deviceId: string, userId: string): Promise<ServiceResult<SessionDbModel>> {
        try {
            const isDeleted = await sessionRepository.delete(deviceId, userId);
            if (!isDeleted){
    ...
    async create(
        ...
        const newEntry = await sessionRepository.create(insertData);
        if (!newEntry){
            return {data: null, status: false}
        }
        return {data: {...insertData, _id: newEntry.insertedId }, status: true}
    },
    ...
    async updateSession(
        ...
        const isUpdated = await sessionRepository.updateSession(
            deviceId,
            {
                issuedAt: issuedAt,
                expireAt: expireAt,
    ...
    async findOne(sessionData: Partial<SessionDbModel>): Promise<ServiceResult<SessionDbModel>>{
        const session = await sessionRepository.findOne(sessionData);
        if(!session){
            return {data: null, status: false, msg: SERVICE_CUSTOM_MSG.NOT_FOUND}
        }
